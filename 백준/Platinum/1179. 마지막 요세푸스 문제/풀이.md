...
### $n$을 줄일 수는 없을까?
$n<k(1 ≤ K ≤ 90)$ 경우 $n$이 크지 않아 아래 점화식을 이용하면 됩니다.
$$g(n, k)=(g(n-1,\ k)+k) \mod{n} \ \mathrm{if} \ 1 < n < k$$ 

문제는 $n>k(1 ≤ N ≤ 10^{15})$이 클 경우 $n$을 어떻게 줄일 수 있느냐입니다. 다행히도 한 바퀴를 돌 경우 $n/k$명이 죽는다는 사실을 착안하여 $n-n/k$로 $n$을 줄여갈 수 있습니다. 이를 점화식으로 표현하면 아래와 같습니다.
...
우리가 구하고자 하는 것은 **마지막으로 살아남는 사람의 번호**입니다. 
