...
def main():
    ...
    def solved(start, end, type):
        ...
        now = 0
        while any(dq):
            while not dq[0]: dq[0], dq[1], dq[2] = dq[1], dq[2], deque(); now += 1
            while dq[0]:
                y, x = dq[0].popleft()
                if dp[y][x] < now: continue
                for dir in range(4):
                    ny = y+dy[dir]; nx = x+dx[dir]
                    if ny < 0 or nx < 0 or ny >= n or nx >= n: continue
                    nd = dp[y][x]+(a[ny][nx] == ['a', 'b'][type^(now&1)]); 
                    if a[ny][nx] == ['b', 'a'][type] and not now: nd = 2
                    if dp[ny][nx] <= nd: continue
                    dp[ny][nx] = nd; dq[nd-now].append((ny, nx))
